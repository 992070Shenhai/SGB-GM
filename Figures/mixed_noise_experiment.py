...
def add_mixed_noise(data, total_noise_ratio, feature_noise_weight=0.5):
    """
    为数据集添加混合噪声，包括特征噪声和标签噪声

    :param data: 输入的数据集，第一列为标签
    :param total_noise_ratio: 总体噪声比例 (0-1)
    :param feature_noise_weight: 特征噪声在总噪声中的权重 (0-1)，默认0.7
    :return: 添加混合噪声后的数据
    """
    # 验证参数
    if not 0 <= total_noise_ratio <= 1:
        raise ValueError("总噪声比例必须在0到1之间")
    if not 0 <= feature_noise_weight <= 1:
        raise ValueError("特征噪声权重必须在0到1之间")

    # 计算各类噪声的比例
    feature_noise_ratio = total_noise_ratio * feature_noise_weight
    label_noise_ratio = total_noise_ratio * (1 - feature_noise_weight)

    # 首先添加特征噪声
    noisy_data = add_feature_noise(data, feature_noise_ratio)

    # 然后添加标签噪声
    final_noisy_data = add_noise_to_label(noisy_data, label_noise_ratio)

    return final_noisy_data


def run_algorithms(X, y, mixed_noise_ratios, algorithms, dataset_path, num_folds=10):
    """
    ...
    for ratio in mixed_noise_ratios:
        print(f"\n特征噪声比率: {ratio}")
        ...
        for fold_idx, (train_index, test_index) in enumerate(kf.split(X, y)):
            ...
            # 为训练集添加特征噪声
            train_data_noisy = add_mixed_noise(
                np.hstack((y_train.reshape(-1, 1), X_train)),
                total_noise_ratio = ratio, feature_noise_weight=0.5
            )
    ...
    # 创建results文件夹（如果不存在）
    results_dir = "Mixed_noise_results_on_20_datasets"
    if not os.path.exists(results_dir):
        os.makedirs(results_dir)

    # 将结果保存到Excel文件
    dataset_name = os.path.splitext(os.path.basename(dataset_path))[0]
    output_file = os.path.join(results_dir, f"{dataset_name}_mixed_noise_results_0.4.xlsx")
...
def main():
    ...
    for dataset_path in datasets:
        ...
        try:
            ...
            # 运行实验
            run_algorithms(X, y, mixed_noise_ratios, algorithms, dataset_path, num_folds=10)
