...
def plot_granules_with_all_samples(X, y, granules):
    ...
    colors = ['#3A5F9B', '#FFB347' , 'purple', 'brown', 'y', 'c', 'm', 'k']
...
data = pd.read_csv('../dataset/fourclass1.csv', sep=' ')  # 从文件中读取数据
...
# 添加噪声
noisy_data = add_noise_to_label(data, noise_ratio=0.1)
...
plt.savefig("updated_noisy.png", dpi=300)
plt.show()
